...
func requireInternalTree(t *testing.T, root, description string) {
	t.Helper()
	if !internalTreePresent(t, root, description) {
		t.Skipf("%s sources are internal-only (export-ignored); contract is enforced in the internal repository", description)
	}
}

// internalTreePresent allows mixed tests to keep checking their public inputs
// while omitting only the export-ignored portion in a public source archive.
// A missing internal source in an internal checkout is always a hard failure.
func internalTreePresent(t *testing.T, root, description string) bool {
	t.Helper()
	...
	if os.IsNotExist(err) {
		public, publicErr := testutil.IsPublicSourceTree(".")
		if publicErr != nil {
			t.Fatalf("identify public source tree: %v", publicErr)
		}
		if !public {
			t.Fatalf("%s source root %s is missing from the internal repository", description, root)
		}
		return false
	}
