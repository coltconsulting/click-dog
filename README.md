...
### Recommended: standard guided install

The standard single-node install is guided and creates a systemd service:
...
The installer always checks its downloaded binary archive against the release
SHA-256 manifest. If `cosign` is on the host, it also authenticates that
manifest against Click-Dog's release workflow; without `cosign`, it warns and
continues with checksum-only verification. `cosign` is recommended, not
required. If an installed `cosign` check fails, installation stops; it never
silently falls back.
Only an explicit rerun with `--dangerously-ignore-cosign` skips publisher
authentication, and the archive SHA-256 must still match.

To authenticate `install.sh` itself before giving it root privileges, follow
the optional [bootstrap verification](https://click-dog.com/verify-releases/#authenticate-installsh-before-sudo).

For an unattended host install, run the downloaded script as
`sudo env CLICKHOUSE_PASSWORD=... bash install.sh install -c <collector-host>:4317 --systemd`.
The [Install Guide](https://click-dog.com/install/) covers the standard host
path and links to focused Ansible, Kubernetes, Docker, update, and verification
guides.

For a pre-verified binary, use `-b /path/to/click-dog` to skip the automatic
download.

### Manual download (verify yourself)

If you cannot run the installer, follow the verified manual recipe at
[Verifying releases manually](https://click-dog.com/verify-releases/#verifying-releases-manually).
It walks through fetching the archive plus the signed `checksums.txt`,
...
| **Backfill** | One-shot historical export over an RFC 3339 range: `backfill --start YYYY-MM-DDT00:00:00Z --end YYYY-MM-DDT00:00:00Z` |
...
`click-dog deploy <kubernetes\|docker\|status\|uninstall>` is a separate
subcommand family that generates deployment files or manages installed state.
See [Operation Modes](https://click-dog.com/modes/#click-dog-deploy-deployment-management).

## Documentation

- [Install Guide](https://click-dog.com/install/): guided single-node install, verification, first value, and the post-install `click-dog` workflow
- [Ansible](https://click-dog.com/ansible/) / [Kubernetes](https://click-dog.com/kubernetes/) / [Docker](https://click-dog.com/docker/): self-contained deployment guides
- [Updating](https://click-dog.com/updating/) / [Verify Releases](https://click-dog.com/verify-releases/): lifecycle and supply-chain verification
...
- [Production Support](https://click-dog.com/support/): community help and direct deployment, integration, and troubleshooting support from Click-Dog's maintainer
