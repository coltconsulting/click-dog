...
func TestHomeQuickStart_ShowsStarterAndAdvancedConfigViews(t *testing.T) {
	requireInternalSite(t)
	...
	text := string(data)
	windowStart := strings.Index(text, `<span class="win-title">click-dog.yaml</span>`)
	if windowStart < 0 {
		t.Fatal("www/home.html is missing the click-dog.yaml window")
	}
	preStart := strings.Index(text[windowStart:], `<pre class="code">`)
	if preStart < 0 {
		t.Fatal("click-dog.yaml window is missing its config block")
	}
	preStart += windowStart
	preEnd := strings.Index(text[preStart:], `</pre>`)
	if preEnd < 0 {
		t.Fatal("click-dog.yaml config block is not closed")
	}
	configBlock := text[preStart : preStart+preEnd]
	...
	for _, required := range []string{
		...
		if !strings.Contains(configBlock, required) {
			t.Errorf("homepage minimal config is missing %q", required)
		}
	}
	for _, routineDefault := range []string{
		`class="key">port</span>`,
		`class="key">username</span>`,
		`class="key">service_name</span>`,
		`class="key">enabled</span>`,
		`class="key">check_interval_s</span>`,
		`class="key">log_level</span>`,
	} {
		if strings.Contains(configBlock, routineDefault) {
			t.Errorf("homepage minimal config exposes routine default %q", routineDefault)
		}
	}

	stripTags := regexp.MustCompile(`<[^>]+>`)
	plainConfig := stripTags.ReplaceAllString(configBlock, "")
	plainConfig = strings.TrimSpace(html.UnescapeString(plainConfig)) + "\n"
	tempDir := t.TempDir()
	configPath := filepath.Join(tempDir, "click-dog.yaml")
	if err := os.WriteFile(configPath, []byte(plainConfig), 0o600); err != nil {
		t.Fatalf("write extracted homepage config: %v", err)
	}
	t.Setenv("CLICKHOUSE_PASSWORD", "test-secret")
	if _, err := config.LoadConfig(configPath); err != nil {
		t.Fatalf("homepage minimal config must load and validate: %v\n%s", err, plainConfig)
	}

	advancedPanelStart := strings.Index(text, `<div class="config-panel" id="config-advanced"`)
	if advancedPanelStart < 0 {
		t.Fatal("homepage is missing the advanced configuration panel")
	}
	advancedPreStart := strings.Index(text[advancedPanelStart:], `<pre class="code">`)
	if advancedPreStart < 0 {
		t.Fatal("advanced configuration panel is missing its config block")
	}
	advancedPreStart += advancedPanelStart
	advancedPreEnd := strings.Index(text[advancedPreStart:], `</pre>`)
	if advancedPreEnd < 0 {
		t.Fatal("advanced configuration block is not closed")
	}
	advancedBlock := text[advancedPreStart : advancedPreStart+advancedPreEnd]
	plainAdvanced := stripTags.ReplaceAllString(advancedBlock, "")
	plainAdvanced = strings.TrimSpace(html.UnescapeString(plainAdvanced)) + "\n"
	...
	for _, advancedPath := range []string{
		...
		if !strings.Contains(text, advancedPath) {
			t.Errorf("homepage is missing advanced configuration path %q", advancedPath)
		}
	}
...
func TestHomeInstallerVerificationLink_LabelInName(t *testing.T) {
	requireInternalSite(t)
	...
	linkPattern := regexp.MustCompile(`<a\b([^>]*\bclass="install-verify"[^>]*)>([^<]+)</a>`)
	match := linkPattern.FindStringSubmatch(string(data))
	if match == nil {
		t.Fatal("homepage is missing the installer verification link")
	}
	attributes, visibleLabel := match[1], strings.TrimSpace(match[2])
	ariaPattern := regexp.MustCompile(`\baria-label="([^"]+)"`)
	ariaMatch := ariaPattern.FindStringSubmatch(attributes)
	if ariaMatch == nil {
		t.Fatal("installer verification link is missing its accessible name")
	}
	spokenLabel := strings.TrimSpace(strings.TrimSuffix(visibleLabel, "→"))
	if !strings.Contains(strings.ToLower(ariaMatch[1]), strings.ToLower(spokenLabel)) {
		t.Errorf("installer verification accessible name %q does not contain visible label %q", ariaMatch[1], spokenLabel)
	}
	if !strings.Contains(attributes, `href="/install/#verifying-releases-manually"`) {
		t.Errorf("installer verification link does not target the manual verification docs: %s", attributes)
	}
}
...
// subheading or sibling, because install.md's uninstall re-download lives in a
// subsection of the same section and would otherwise mask deletion of the
...
func TestInstallerBootstrap_IsVerifiedAndInstallRolesAreClear(t *testing.T) {
	...
	gettingStartedText := assertStandardInstallPath(t, "docs/getting-started.md", "", true)
	installGuideText := assertStandardInstallPath(t, "docs/install.md",
		"## Standard Install (Guided, Single Node)", false)

	optionalHeading := strings.Index(installGuideText, "### Optional: authenticate `install.sh` before sudo")
	if optionalHeading < 0 {
		t.Fatal("install guide is missing optional authenticated bootstrap procedure")
	}
	optionalProcedure := installGuideText[optionalHeading:]
	for _, required := range []string{
		"(\n  set -euo pipefail",
		"set -euo pipefail",
		"cosign verify-blob",
	...
	for _, required := range []string{
		...
		if !strings.Contains(gettingStartedText, required) {
			t.Errorf("getting started must explain installer roles and checks: missing %q", required)
		}
	}
}
