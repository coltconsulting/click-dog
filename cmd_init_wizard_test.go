...
func TestBlacklistOpsConsistentAcrossProfiles(t *testing.T) {
	...
	if internalTreePresent(t, filepath.Join("docs", "examples"), "documentation examples") {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.wantConfig {
				...
				// Copy only the rendered YAML, then fetch the current installer on the
				// target. The installer chooses the target architecture and verifies the
				// downloaded archive checksum without making cosign a prerequisite.
				if !strings.Contains(body, "scp "+tc.outputPath+" HOST:/tmp/") {
					t.Errorf("step 5 should copy the rendered YAML to the target, got:\n%s", body)
				}
				if !strings.Contains(body, "releases/latest/download/install.sh -o /tmp/install.sh") {
					t.Errorf("step 5 should download the current installer on the target, got:\n%s", body)
				}
				if !strings.Contains(body, "Cosign is recommended, not required") {
					t.Errorf("step 5 should explain optional publisher authentication, got:\n%s", body)
				}
				if strings.Contains(body, "scp "+tc.outputPath+" install.sh") {
					t.Errorf("step 5 should not copy a potentially stale installer, got:\n%s", body)
				}
